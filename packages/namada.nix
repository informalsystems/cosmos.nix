{
...
}:
  {
    ...
    namada-wasm-scripts = pkgs.rustPlatform.buildRustPackage {
      pname = "namada-wasm-scripts";
      version = "v0.28.1";
      src = pkgs.symlinkJoin 
        { 
         name = "namada-wasm-scripts-src" ;
         paths = [ "${namada-src}/wasm" "${namada-src}/tx_prelude" "${namada-src}/vp_prelude" ];
         patches = [];
        };
      nativeBuildInputs = with pkgs;
        (
      ...
      preBuild = "${pkgs.tree}/bin/tree .";

      cargoLock = {
        lockFile = "${namada-src}/wasm/Cargo.lock";
        outputHashes = {
          "borsh-ext-1.2.0" = "sha256-nQadqyeAY0/gEMLBkpqtSm5D7kV+r3LVT/Cg2oTV7Ug=";
          "clru-0.5.0" = "sha256-/1NfKqcWGCyF3+f0v2CnuFmNjjKkzfvYcX+GzxLwz7s=";
          "ethbridge-bridge-contract-0.24.0" = "sha256-qs81bIWKk4oxh6nFWqAt4eBbPuIWF2a3ytUSjDJZWSU=";
