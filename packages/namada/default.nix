...
in
  ...
  namada-wasm-scripts = let
    craneLib' = craneLib.overrideToolchain (pkgs.rust-bin.selectLatestNightlyWith (toolchain: toolchain.default));
    src = namada-src;
    cargoExtraArgs = "--manifest-path=./wasm/Cargo.toml --locked";
    name = "namada-wasm-scripts";
    cargoArtifacts = craneLib'.buildDepsOnly {
      cargoLock = "${src}/wasm/Cargo.lock";
      pname = "${name}-deps";
      inherit version src cargoExtraArgs;
    };
  in craneLib'.buildPackage {
    pname = name;
    inherit version src cargoExtraArgs cargoArtifacts;
  };
}
