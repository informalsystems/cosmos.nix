{
  ...
  buildApp = dependencyRegex: args@{
    ...
    dependency-version = with nix-std.lib; let
      all-dep-matches =
        regex.allMatches
        "${dependencyRegex}[[:space:]](=>[[:space:]]?[[:graph:]]*[[:space:]])?v?[[:graph:]]*"
    ...
    in
      optional.match dep-version {
        nothing =
          ...
          "Could not find a ${dependencyRegex} version with regex, check if the formatting of go.mod escapes the regex in cosmos.nix/resources/utilities"
          null;
        just = function.id;
      };
  ...
  in
    pkgs.buildGo119Module ({
        ...
        ldflags = ''
          ...
          -X github.com/${dependencyRegex}/${dependencyRegex}/version.TMCoreSemVer=${dependency-version}
          ${additionalLdFlags}
        '';
      }
      // buildGoModuleArgs);
