  ...
  mkCosmosGoAppComet = {
    ...
    buildGoModuleArgs =
      ...
      (n: _:
        ...
        ["src" "name" "version" "vendorSha256" "appName"])
      args;
    cometbft-version = with nix-std.lib; let
      all-tm-matches =
        regex.allMatches
        "cometbft/cometbft[[:space:]](=>[[:space:]]?[[:graph:]]*[[:space:]])?v?[[:graph:]]*"
        (builtins.readFile "${src}/go.mod");
      tm-string =
        if list.any (string.hasInfix "=>") all-tm-matches
        then list.head (list.filter (string.hasInfix "=>") all-tm-matches)
    ...
    in
      optional.match tm-version {
        nothing =
          pkgs.lib.trivial.warn
          "Could not find a cometbft version with regex, check if the formatting of go.mod escapes the regex in cosmos.nix/resources/utilities"
          null;
        just = function.id;
      };
  ...
  in
    pkgs.buildGo119Module ({
        ...
        ldflags = ''
          ...
          -X github.com/cosmos/cosmos-sdk/version.Version=${version}
          -X github.com/cosmos/cosmos-sdk/version.Commit=${src.rev}
          -X github.com/cometbft/cometbft/version.TMCoreSemVer=${cometbft-version}
          ${additionalLdFlags}
        '';
      }
      // buildGoModuleArgs);
